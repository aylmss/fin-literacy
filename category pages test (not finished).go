...
func updatePayment(w http.ResponseWriter, r *http.Request) {
	...
	json.Unmarshal(reqBody, &updatedPayment)
	for i, payment := range Payments {
		if payment.Id == id {

			payment.Title = updatedPayment.Title
			payment.Price = updatedPayment.Price
			payment.Date = updatedPayment.Date
			payment.Type = updatedPayment.Type
			payment.Comment = updatedPayment.Comment
			payment.Category = updatedPayment.Category
...
func returnSingleCategory(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	key := vars["id"]

	for _, payment := range Payments {
		if string(payment.Category) == key {
			json.NewEncoder(w).Encode(payment)
		}
	}
}
...
func handleRequests() {
	...
	myRouter.HandleFunc("/categories/{category}", returnSingleCategory).Methods("GET")
...
func main() {
	...
	// var paymentCategories = map[int]string{
	// 	1: "work",
	// 	2: "house",
	// 	3: "food",
	// }
	...
	Payments = []Payment{
		Payment{Id: "1", Title: "Salary", Price: 120,
			...
			Type: "Income", Comment: "comment for id1", Category: 1},
		Payment{Id: "2", Title: "Rent", Price: 50,
			Date: time.Date(2022, time.February, 18, 11, 15, 0, 0, time.UTC),
			Type: "Expense", Comment: "comment for id2", Category: 2},
		Payment{Id: "3", Title: "Magnum", Price: 20,
			Date: time.Date(2022, time.February, 15, 13, 50, 0, 0, time.UTC),
			Type: "Expense", Comment: "comment for id3", Category: 3},
	}
	fmt.Println("Statement of current financial position: ", Total(Payments))
